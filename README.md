...
```
├── YYYY-MM-DD.html          # 每期报纸（根目录，日期版不进仓库；<head> 只引用 css/site.css）
├── css/site.css             # 【外观】全刊唯一样式表：字体、颜色、栏宽、灰度都在这
├── docs/                    # Pages 站点（生成物：index.html + css/ + 最新一期 img/，别手改）
├── 更新首页.py               # 最新一期 → docs/（配图压成 JPEG，样式表随附）
├── img/YYYY-MM-DD/          # 见报图（公开成图 / AI 生成）
│
├── QWEN.md                  # 【流程】主编身份 + 六步顺序 + 谁读哪条规则
├── rules/                   # 【标准】唯一来源，一条主题一个文件
│   ├── lines.md             #   R1 四条线采集侧重与配额、跨线归属判定
│   ├── volume.md            #   R2 版号分区、每期版数、字数下限、重稿门槛
│   ├── prose.md             #   R3 文体纪律（八条 + 真实反例句）
│   ├── visual.md            #   R4 样式表位置、数字写法、成图与生成图口径、类名
│   ├── page-size.md         #   R5 纸宽/栏数/高宽比合格线、导出脚本回吐字段
│   └── disk.md              #   R6 路径模板 + 写盘方法（非 Windows 改这里）
│
├── .qwen/
│   ├── agents/              # 四个子代理：只写「读哪条规则 + 怎么执行 + 输出格式」
│   │   ├── fetch.md         #   取材：预算、单页纪律、原文入库、素材文件格式
│   │   ├── copy-check.md    #   校对：grep 骨架与标签，标准指向 R2/R3
│   │   ├── image-check.md   #   配图质检：只读 R4，回「通过」或「问题：」
│   │   └── layout-check.md  #   版面验收：跑导出脚本 + 判尺寸，标准指向 R5
│   ├── skills/
│   │   ├── fetch-poster/    # 海报直链下载技能（og:image + 绕防盗链 + 验真图）
│   │   └── layout-review/   # 按版截图脚本（纸宽/纸色从样式表读，顺带回吐每版尺寸）
│   └── settings.json        # 项目级设置
...
| 想改什么 | 只改这个文件 |
|---|---|
| 选题方向、四条线、各线采集配额与跨线归属（例：把「动漫」换成「体育」） | `rules/lines.md`（R1）——表里的线名同时是派工名与素材文件名；换选题域还要把 `desk/数据源.md` 的站表换成对应媒体（见第四层） |
| 每期几版、每版多少字、单条稿下限、重稿门槛、版号字母分区 | `rules/volume.md`（R2） |
| 文风（禁用句式与每版配额、不许出现「本期/未获取清单」这类交代、署名规则） | `rules/prose.md`（R3） |
| 字体、数字写法、图必须黑白、图注三种口径、人像必须像本人 | `rules/visual.md`（R4，口径）＋ `css/site.css`（实现） |
| 纸宽、栏数、高宽比合格线（嫌版面太挤或太空就调这里） | `rules/page-size.md`（R5，判定线）＋ `css/site.css` 里 `.page` 与 `.flow`（实现，同一文件两处） |
| 工作稿存放位置、文件名规则、非 Windows 的写盘与命令写法 | `rules/disk.md`（R6） |

改 `rules/` 是改**标准**，改 `css/site.css` 是改**实现**，两处各有分工、互不抄写：R4 与 R5 只告诉代理"外观在样式表里、判定线在这"，具体像素与字体栈由样式表承担，导出脚本运行时反过来从样式表读纸宽与纸色——所以没有第三份常数。

改完之后：**下一期**的 HTML 与导出会照新标准生成；已出版的历史那几期是各自独立的文件，要改得手改（`docs/` 里的发布副本则由 `更新首页.py` 重新覆盖）。
...
| 校对多查一类问题 | `.qwen/agents/copy-check.md` 的 grep 骨架与标签（禁词清单本体在 `rules/prose.md`） |
| 子代理用哪个模型 | 各代理文件顶部 `model:`（改档位还要看 `.qwen/settings.json` 的 `allowedGrades`） |
| 版面导出细节（浏览器路径、默认倍率、窗口高＝溢出探测上限） | `.qwen/skills/layout-review/review.py`；参数 `--html` `--output`（必填，路径模板取自 R6）、`--pages`、`--width`、`--scale` |
...
报纸的字体栈、颜色变量、纸宽、栏数、图片灰度、表格线**全在这张表里**，每期 HTML 只留一行 `<link>` 引它。想换 serif／sans、换纸色、加一条细线，改这里就够，不用碰任何提示词。

`docs/` 是生成物，由 `更新首页.py` 覆盖（它会连带把 `css/site.css` 复制到 `docs/css/`），别手改；只有生成逻辑要变（比如首页不再用最新一期、想加目录页）才改那个脚本。

## 声明

本项目的工作流运行依赖开源命令行工具 **[Qwen Code](https://github.com/QwenLM/qwen-code)**（官方仓库：<https://github.com/QwenLM/qwen-code>）。`QWEN.md`、`.qwen/agents/`、`.qwen/settings.json` 均为 Qwen Code 的项目配置格式；`rules/` 是本仓库自定的普通 Markdown，靠 `QWEN.md` 里的 `@rules/×.md` 引用加载——已在 v0.22.3 实测：新会话里禁止任何工具，模型仍能答出 R2 的字数门槛，说明规则确实自动进上下文。报纸内容含 AI 生成成分，图片注明「公开资料」或「人工智能生成／本报制图」，不代表任何真实媒体机构。
